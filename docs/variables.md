...
- [x] ExecuteManager への統合（プロンプト実行時の変数展開）
  ...
  - [x] 変数入力ダイアログの表示
  - [x] プロンプトの展開と実行
- [x] インポート/エクスポート機能への対応
  - [x] `PromptCSVRow` 型に `variables` フィールド追加
  - [x] エクスポート時に `variables` フィールドをJSON文字列として含める (`promptExportService.ts`)
  - [x] インポート時に `variables` フィールドをJSONから復元 (`promptImportService.ts`)
  - [x] 後方互換性の確保（変数なしのCSVも正常にインポート可能）
  - [x] ユニットテストの追加
    - [x] エクスポート機能のテスト (16ケース)
    - [x] インポート機能のテスト (21ケース)
- [x] テストの追加
  - [x] EditDialog の変数設定UI部分のユニットテスト (15ケース)
    - 変数の自動検出とUI生成
    - 変数設定の永続化とマージ
    - 保存時の変数設定の含有
    - ダイアログの状態管理
  - [ ] InputPopup の変数展開フローのユニットテスト（統合テストの複雑さにより保留）
    - テストケースは実装済みだが、モック設定が複雑で一旦スキップ
    - 実際の機能は正常に動作していることを確認済み
